...
try: from dotenv import set_key, find_dotenv # Using set_key might be cleaner for updating later
...
def get_available_models(api_key):
    ...
    try:
        ...
        usable_models = sorted([m.name for m in models if 'generateContent' in m.supported_generation_methods]) # Sort for consistent display
        ...
        model_ids = [name.replace('models/', '') for name in usable_models] # Get short names
...
def main():
    ...
    if args.non_interactive:
        ...
        else: print(f"Error: Specified model '{model_name_arg}' unavailable.\nAvailable: {', '.join(available_models)}", file=sys.stderr); sys.exit(1)
        ...
        except ValueError: pass # Default not in list
        prompt = f"Select model number [{DEFAULT_MODEL_NAME}]: "
        while not final_model_name:
            try:
                choice_str = input(prompt).strip()
                if not choice_str and default_choice_num != -1: choice = default_choice_num # Use default index if input empty & default valid
                elif not choice_str: choice = 1 # Fallback to first if input empty & default invalid
    ...
    if not args.non_interactive:
        ...
        print(f"Owner/Permissions:   {SERVICE_USER}:{SERVICE_GROUP} / {oct(ENV_PERMS)[2:]o}") # Show octal perms
    ...
    try:
        ...
        if 'temp_path' in locals() and os.path.exists(temp_path): try: os.remove(temp_path); except OSError: pass
        sys.exit(1)
